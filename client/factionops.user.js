...
`;
    ...
    function connectRealtime() {
        if (realtimeSocket || sseConnected) return; // already connected or connecting
        if (!state.jwtToken) return;

        // NEW STRATEGY (v4.7.2): Bypass Page CSP
        // Standard Socket.IO (XHR/Websocket) is blocked by Torn's connect-src CSP.
        // SSE via GM_xmlhttpRequest is the ONLY way to get real-time through the sandbox on desktop.
        if (canUseSSEStream()) {
            log('CSP-bypass environment detected. Using SSE Stream as primary RT transport.');
            connectSSEStream();
            return;
        }

        // Fallback for environments without GM_xmlhttpRequest (rare)
        if (IS_PDA) {
