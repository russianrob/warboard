...
<div class="card"><h2>How it's stored</h2><ul><li>Plaintext JSON files on the local disk of a single dedicated server in <strong>Helsinki, Finland</strong> (Hetzner, AS24940). Single-tenant hardware — no other customers on the machine.</li><li>File permissions restrict read access to the machine's root user only. No other Unix user, process, or external system can access the files.</li><li>The server's root user can be accessed only via SSH with a hardware security key (FIDO2) plus a rotating 6-digit TOTP code. Password SSH login is disabled.</li><li>HTTPS (TLS 1.3) protects all traffic between your browser and the server.</li><li>Per-faction data is separated: either as dedicated files named with your faction ID, or as namespaced entries in shared files keyed by faction ID. Faction identity is always derived from Torn's own <code>/user</code> endpoint response, never from user input.</li><li><em>Optional, on request</em>: the operator can enable passphrase-based encryption at rest. With this enabled, a disk snapshot (backup theft, rescue-boot attack) yields only ciphertext.</li></ul></div>
