...
(function () {
    ...
    const S = {
        ...
        poolOptIn:      store.get('w3_pool_optin', true),   // default on
        poolSize:       null,                               // filled from /status
        settingsOpen:   false,
        realDeals:      [],
    ...
    // ── Verify-pool opt in/out ───────────────────────────────────────────────
    // Contributes the user's key to the server-side round-robin verify
    // pool. Key is only used for public user/bazaar lookups. Default on
    // when a user saves their API key; explicit opt-out removes it.
    ...
    async function poolOptIn(apiKey) {
        try {
            const r = await gmPostJSON(`${WARBOARD_BASE}/api/weav3r/pool/opt-in`, { key: apiKey });
            if (r && r.ok) {
                S.poolOptIn = true;
                S.poolSize  = r.poolSize;
                store.set('w3_pool_optin', true);
                if (S.settingsOpen) render();
            }
        } catch (_) { /* silent — opt-in is best-effort */ }
    }
    async function poolOptOut(apiKey) {
        try {
            const r = await gmPostJSON(`${WARBOARD_BASE}/api/weav3r/pool/opt-out`, { key: apiKey });
            if (r && r.ok) {
                S.poolOptIn = false;
                S.poolSize  = r.poolSize;
                store.set('w3_pool_optin', false);
                if (S.settingsOpen) render();
            }
        } catch (_) { /* silent */ }
    }
    async function refreshPoolStatus(apiKey) {
        if (!apiKey) return;
        try {
            const r = await gmJSON(`${WARBOARD_BASE}/api/weav3r/pool/status?key=${encodeURIComponent(apiKey)}`);
            if (r && typeof r.optedIn === 'boolean') {
                S.poolOptIn = r.optedIn;
                S.poolSize  = r.poolSize;
                store.set('w3_pool_optin', r.optedIn);
                if (S.settingsOpen) render();
            }
        } catch (_) { /* silent */ }
    }
    ...
    function renderSettings() {
        ...
        return `
            <div style="padding:12px;">
                ...
                <div class="w3b-section-label" style="margin-top:16px;">Verify Pool</div>
                <div class="w3b-hint" style="margin-bottom:10px;color:#778;">Contribute your key to the shared verify pool. The server rotates through opted-in keys to check which $1 listings are still buyable, so no single key burns its 100/min Torn API budget. <b>Public bazaar lookups only</b> — pooled keys are never used for anything private. Opt out any time; stays on by default.</div>
                <label style="display:flex;align-items:center;gap:8px;cursor:pointer;user-select:none;">
                    <input id="w3b-pool-input" type="checkbox" ${S.poolOptIn ? 'checked' : ''}
                        ${S.apiKey ? '' : 'disabled'}
                        style="width:18px;height:18px;cursor:${S.apiKey ? 'pointer' : 'not-allowed'};">
                    <span style="color:${S.apiKey ? '#dde' : '#778'};font-size:13px;">Share my key with the Weaver verify pool</span>
                </label>
                ${!S.apiKey ? '<div style="margin-top:6px;font-size:11px;color:#778;">Save an API key first to opt in.</div>' : ''}
                ${S.poolSize != null ? `<div style="margin-top:6px;font-size:11px;color:#9ca3af;">Current pool: <b style="color:#4ade80;">${S.poolSize}</b> contributor${S.poolSize === 1 ? '' : 's'}</div>` : ''}
    ...
    function attachListeners() {
        ...
        if (saveBtn) saveBtn.addEventListener('click', () => {
            ...
            if (val) {
                ...
                // Auto-enroll in the verify pool if the user hasn't
                // explicitly opted out. Default-on per product decision.
                if (S.poolOptIn) poolOptIn(val);
                else refreshPoolStatus(val);
            }
            render();
        });
        const clearBtn = body.querySelector('#w3b-apikey-clear');
        if (clearBtn) clearBtn.addEventListener('click', () => {
            const oldKey = S.apiKey;
            S.apiKey = '';
            store.set('w3_apikey', '');
            S.verified = {};
            S.settingsOpen = false;
            // Remove from pool when the user clears their key so we
            // don't keep a stale key around server-side.
            if (oldKey) poolOptOut(oldKey);
            render();
        });

        const poolCb = body.querySelector('#w3b-pool-input');
        if (poolCb) poolCb.addEventListener('change', (e) => {
            const checked = !!e.target.checked;
            S.poolOptIn = checked;
            store.set('w3_pool_optin', checked);
            if (!S.apiKey) return;
            if (checked) poolOptIn(S.apiKey);
            else         poolOptOut(S.apiKey);
        });
    ...
    function buildPanel() {
        ...
        head.querySelector('#w3b-settings-btn').addEventListener('click', () => {
            ...
            // Sync checkbox state with server truth every time the
            // panel opens, so toggling off in another tab is reflected.
            if (S.settingsOpen && S.apiKey) refreshPoolStatus(S.apiKey);
            render();
        });
    ...
    function init() {
        ...
        // Auto-enroll in the verify pool on startup if the user has a
        // saved key and hasn't explicitly opted out. Defers 4s so we
        // don't compete with buildIndexFromTornAPI for the first burst
        // of Torn API calls.
        if (S.apiKey) setTimeout(() => {
            if (S.poolOptIn) poolOptIn(S.apiKey);
            else             refreshPoolStatus(S.apiKey);
        }, 4000);
    }
